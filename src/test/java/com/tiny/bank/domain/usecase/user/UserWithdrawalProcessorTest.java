...
class UserWithdrawalProcessorTest {
    ...
    private UserWithdrawalProcessor victim;
    ...
    void setUp() {
        ...
        victim = new UserWithdrawalProcessor(repositoryMock);
    }

    @Test
    void shouldTestTheWithdrawal() {
        ...
        verify(userMock).withdraw(accountMock.getAccountId(), BigDecimal.TEN);
    }


    @Test
    void shouldTestTheUserWithdrawalWhenUserDoesntExist() {
        when(accountMock.getAccountId()).thenReturn(UUID.randomUUID());
        ...
        verify(userMock, never()).withdraw(any(), any());
    }
